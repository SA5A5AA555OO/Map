...
const searchStore = async (favorite) => {
  try {
    const querySnapshot = await getDocs(query(collection(db, 'store'), where('store_name', '==', Favorite)));
    if (!querySnapshot.empty) {
      const storeDocs = querySnapshot.docs.map(doc => doc.data());
      setStoreData(storeDocs);
      setStoreName(storeDocs[0].store_name);
      console.log('Store Data:', storeDocs);
    } else {
      console.log('No documents found for query');
    }
  } catch (error) {
